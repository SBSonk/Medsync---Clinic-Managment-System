...
def get_emergency_contact(patient_id):
    p = (
        db.session.query(models.EmergencyContact)
        .filter_by(patient_id=patient_id)
        .first()
    )
    if p is not None:
        return jsonify({"person_id": p.person_id, "relation": p.relation}), 200
    else:
        return (
            jsonify(
                {
                    "message": f"emergency contact with patient_id ({patient_id}) not found."
                }
            ),
            404,
        )
...
def get_inventory_info(inventory_id):
    ...
    if item is not None:
        ...
        return (
            jsonify({"message": f"Inventory item with ID ({inventory_id}) not found."}),
            404,
        )
